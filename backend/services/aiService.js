 ...
 * Returns a list of issues matching the frontend's IssueResult shape.
 */
async function reviewSnippet(code, language, reviewType, focusAreas) {
  const activeFocusLabels = {
  ...
  const focusList = focusAreas
    ? Object.keys(focusAreas)
        .filter((key) => focusAreas[key] && activeFocusLabels[key])
        .map((key) => activeFocusLabels[key])
    : [];
  const focusLine = focusList.length
    ? `\nPay particular attention to: ${focusList.join(", ")}.`
    : "";
  ...
  const messages = [
    ...
    {
      role: "user",
      content: `Language: ${language}\nReview type: ${reviewType}${focusLine}\n\n\`\`\`${language}\n${code}\n\`\`\``,
    },
  ];
