...
───────────────────────────────────────────── */
function LanguageBadge({ language }: { language?: string }) {
  // Fixed: previously spread in LANGUAGE_COLORS.default, a dark-theme class
  // ("text-zinc-300") meant for dark backgrounds — on this light glass card
  // it made the text nearly invisible. The badge already defines its own
  // correct light-theme colors below, so nothing else is needed.
  return (
    <span className="inline-flex items-center gap-1.5 rounded-full border border-violet-200/60 bg-white/60 px-2.5 py-1 text-[11px] font-medium text-[#1F1B3D] backdrop-blur-sm">
      <span className="h-1.5 w-1.5 rounded-full bg-violet-400" />
      Auto-detected
    </span>
  );
