...
function setCookie(name: string, value: string, days?: number) {
  // When `days` is omitted we set a *session* cookie (no expires attribute),
  // which the browser clears once it's closed — used when "remember me" is off.
  const expiryPart = days ? `;expires=${new Date(Date.now() + days * 864e5).toUTCString()}` : "";
  document.cookie = `${name}=${value}${expiryPart};path=/;SameSite=Lax`;
}
function delCookie(name: string) {
  document.cookie = `${name}=;expires=Thu, 01 Jan 1970 00:00:01 GMT;path=/`;
}
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const signIn = useCallback(async (email: string, password: string, remember = true) => {
    ...
    // "Remember me" checked -> persistent 30-day cookie.
    // Unchecked -> session cookie, cleared when the browser closes.
    setCookie("auth-token", data.jwt, remember ? 30 : undefined);
    setUser(mapStrapiUser(data.user));
  }, []);
